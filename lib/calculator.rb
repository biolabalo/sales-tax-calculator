class Calculator
  ...
  def update_amounts
    @items = @items.each do |item|
      item[:applicable_tax] = set_tax(item[:taxable], item[:total], @applicable_tax_rate)
      item[:import_tax] = set_tax(item[:import], item[:total], @import_tax_rate)
      item[:sales_tax] = add_taxes(item[:sales_tax], item[:applicable_tax], item[:import_tax])
      item[:total] = add_taxes(item[:total], item[:applicable_tax], item[:import_tax])
    end
  end
 ...
 def set_tax(status, base_total, tax_rate)
  ...
  def add_taxes(amt, applicable_tax, import_tax)
    amt += applicable_tax + import_tax
    amt.round(2)
  end
